...
with con:
    while start_date < end_date:
        cur.execute("INSERT INTO daily_maxtemp(day_of_reading) VALUES (?)", (int(start_date.strftime('%Y%m%d')),))
        start_date += datetime.timedelta(days=1)

#loop through the cities and query the API
for k,v in cities.iteritems():
    start_date = end_date - datetime.timedelta(days=30) #reset value each time through the loop 
    while start_date < end_date:

        #query for the value
        r = requests.get("https://api.forecast.io/forecast/8e2f0f9e97727516429316ac1aaec3ad/"+ v + "," + str(start_date.strftime('%Y-%m-%dT%H:%M:%S')))
